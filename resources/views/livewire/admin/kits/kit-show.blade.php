<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Kit: {{ $kit->kit_number }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Detalle del kit de producción</p>
        </div>
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-lg">
            <div class="p-6">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <dt class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Número de Kit</dt>
                        <dd class="text-sm text-gray-900 dark:text-white font-semibold">{{ $kit->kit_number }}</dd>
                    </div>
                    ...
                    <div>
                        ...
                        <dd class="mt-1">
                            ...
                            @endphp
                            <button wire:click="openStatusModal" class="px-3 py-1 text-xs font-semibold rounded-full {{ $statusColors[$kit->status] ?? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 border-2 border-gray-200 dark:border-gray-600' }} hover:opacity-80 cursor-pointer transition-opacity">
                                {{ $kit->status_label }}
                            </button>
                        </dd>
                    </div>
                    ...
                    <div>
                        <dt class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Validado</dt>
                        <dd class="mt-1">
                            @if($kit->validated)
                    ...
                    @if($kit->preparedBy)
                    <div>
                        <dt class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Preparado por</dt>
                    ...
                    @if($kit->releasedBy)
                    <div>
                        <dt class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Liberado por</dt>
                ...
                @if($kit->validation_notes)
                    <div class="mt-6 pt-4 border-t-2 border-gray-200 dark:border-gray-700">
                        <dt class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Notas de Validación</dt>
                    ...
                    <div class="space-y-3">
                        ...
                        @if($kit->validated && $kit->status === 'preparing')
                            <button wire:click="invalidateKit"
                                class="w-full inline-flex justify-center items-center gap-2 px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white text-sm font-medium rounded-md">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($kit->lots->count() > 0)
                        <div class="space-y-2">
                            @foreach($kit->lots as $lot)
                                <div class="p-3 bg-gray-50 dark:bg-gray-900/50 border-2 border-gray-200 dark:border-gray-700 rounded-lg">
                                    <div class="flex items-center justify-between">
                                        <a href="{{ route('admin.lots.show', $lot) }}" class="text-sm font-semibold text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                                            {{ $lot->lot_number }}
                                        </a>
                                        @php
                                            $lotStatusColors = [
                                        ...
                                        @endphp
                                        <span class="px-2 py-0.5 text-xs font-semibold rounded-full {{ $lotStatusColors[$lot->status] ?? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 border-2 border-gray-200 dark:border-gray-600' }}">
                                            {{ $lot->status_label }}
                                        </span>
                                    </div>
            ...
            @if($kit->incidents->count() > 0)
                <div class="space-y-4">
                    @foreach($kit->incidents as $incident)
                        <div class="p-4 rounded-lg border-2 {{ $incident->resolved ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-700' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-700' }}">
                            <div class="flex justify-between items-start">
                                <div>
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border-2 {{ $incident->resolved ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border-green-200 dark:border-green-700' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 border-red-200 dark:border-red-700' }}">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border-2 border-gray-200 dark:border-gray-700">
                    ...
                    <div class="px-6 py-4 border-t-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 sm:flex sm:flex-row-reverse">
                        <button wire:click="saveIncident"
                            class="w-full inline-flex justify-center items-center gap-2 rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-sm font-medium text-white hover:bg-red-700 sm:ml-3 sm:w-auto">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                            Registrar Incidencia
