        ...
        <div class="lg:col-span-1 space-y-4">
            <div class="rounded-lg bg-gray-50 dark:bg-gray-900 p-4">
                ...
                <div class="mb-4">
                    ...
                    />
                    @error('currentQuantity') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                </div>
                ...
                @if($parts->isEmpty())
                    <div class="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                        ...
                        <a href="{{ route('admin.standards.create') }}" class="text-xs text-blue-600 hover:underline mt-1 inline-block">
                            Crear Estándar →
                        </a>
                    </div>
                @endif
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-4 text-center">
                    ...
                    <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ number_format($totalAvailableHours, 2) }}</p>
                </div>
                <div class="rounded-lg bg-orange-50 dark:bg-orange-900/20 p-4 text-center">
                    ...
                    <p class="text-2xl font-bold text-orange-600 dark:text-orange-400">{{ number_format($totalRequiredHours, 2) }}</p>
                </div>
                <div @class([
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-yellow-700 dark:text-yellow-300">
                                Se requieren <strong>{{ number_format($suggestedOvertime, 2) }} horas</strong> adicionales.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($workOrderItems as $index => $item)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900 dark:text-white">{{ $item['part_number'] }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ Str::limit($item['part_description'] ?? '', 25) }}</div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <button 
                                        wire:click="removeWorkOrderItem({{ $index }})"
                                        type="button"
                    ...
                    @forelse($this->availablePOs as $po)
                        ...
                        <div @class([
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <input 
                                    type="checkbox" 
                                    wire:click="togglePOSelection({{ $po->id }})"
                                    @checked($isSelected)
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center justify-between mb-2">
                                        <div>
                                            <span class="font-semibold text-gray-900 dark:text-white">{{ $po->po_number }}</span>
                                            <span class="mx-2 text-gray-400">|</span>
                                            <span class="text-gray-700 dark:text-gray-300">{{ $po->part->number }}</span>
                                        </div>
                                        <span class="text-sm text-gray-500">Qty: {{ number_format($po->quantity) }}</span>
                                    </div>
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">{{ $po->part->description }}</p>
                                                ...
                                                @foreach($configurations as $config)
                                                    ...
                                                    <label @class([
                                                        ...
                                                        <input 
                                                            ...
                                                            @disabled(!$canUse)
                                                            @checked(isset($poConfigurations[$po->id]) && $poConfigurations[$po->id] == $config->id)
                                                            class="mr-2"
                                                        />
