        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $productionStatus->name }}</h1>
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            Detalles del estado de producción
                        </p>
                    </div>
                </div>
                <a href="{{ route('admin.production-statuses.edit', $productionStatus) }}"
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white flex items-center gap-3">
                            <div class="w-3 h-3 rounded-full" style="background-color: {{ $productionStatus->color }}"></div>
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white flex items-center gap-2">
                            <div class="w-8 h-8 rounded border border-gray-300 dark:border-gray-600" style="background-color: {{ $productionStatus->color }}"></div>
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $productionStatus->order }}</dd>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                            {{ $productionStatus->description ?? 'Sin descripción' }}
                        </dd>
                    </div>

                    <div>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Fecha de Creación</dt>
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $productionStatus->created_at->format('d/m/Y H:i') }}</dd>
                    </div>

                    <div>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Última Actualización</dt>
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $productionStatus->updated_at->format('d/m/Y H:i') }}</dd>
                    </div>
                </dl>
            </div>
        </div>
